    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
        ...
        <motion.div
          ...
          className="relative bg-white w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden z-10 border border-slate-200 my-8 max-h-[90vh] flex flex-col"
        >
          {/* Header Bar */}
          <div className="flex items-center justify-between px-6 py-4 bg-brand-forest text-white border-b border-brand-gold/30 shrink-0">
            ...
            <button
              onClick={onClose}
              className="p-1.5 rounded-full bg-white/10 hover:bg-white/20 text-slate-200 hover:text-white transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="p-6 overflow-y-auto space-y-6">
            ...
            <div className="relative h-64 rounded-xl overflow-hidden border border-slate-200">
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-brand-dark/90 via-brand-dark/30 to-transparent flex items-end p-6">
                <div>
            ...
            {/* Specs Grid */}
            <div className="bg-brand-cream rounded-xl p-4 border border-slate-200">
              <h5 className="text-xs font-bold text-brand-forest uppercase tracking-wider mb-3 flex items-center">
                <FileText className="w-4 h-4 mr-1.5 text-brand-gold" /> Verified Quality & Export Specifications
              </h5>
              <div className="grid sm:grid-cols-2 gap-3">
                {product.specs.map((spec, i) => (
                  <div key={i} className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm flex flex-col">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {product.origins.map((origin, i) => (
                    <span key={i} className="px-3 py-1 bg-emerald-50 text-emerald-800 text-xs font-bold rounded-lg border border-emerald-200">
                      {origin}, Tanzania
                    </span>
                  ))}
                </div>
              ...
              <div className="space-y-2">
                {product.highlights.map((item, i) => (
                  <div key={i} className="flex items-start text-xs text-slate-600">
                    <CheckCircle className="w-4 h-4 text-emerald-600 mr-2 shrink-0 mt-0.5" />
                    <span>{item}</span>
                  </div>
                ))}
              </div>
